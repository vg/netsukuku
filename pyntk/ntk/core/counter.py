...
from ntk.lib.crypto import fnv_32_buf, md5, verify, public_key_from_pem
...
class Counter(P2P):
    ...
    def check(self, public_key, hostname, signature, IDNum):
        ...
        if self.cache.has_key(public_key) and len(self.cache[public_key]) > self.max_hostnames_limit:
            raise CounterError("Hostnames limit reached")        
        if not self.cache.has_key(public_key):
            self.cache[public_key] = {}
        if self.cache.has_key(public_key) and self.cache[public_key].has_key(hostname):
            ...
            if updates == IDNum:
                ...
                self.forward_registration(public_key, hostname, self.cache[public_key][hostname])
            ...
            for id in xrange(self.maproute.gsize):
                ...
                if self.maproute.nip_cmp(nip, me) == 0 and \
                    self.mapp2p.node_get(lvl, id).participant:    
                        ...
                        res, data = remote.reply_forward_registration(public_key, hostname, entry)
    ...
    def h(self, pubk):
        ...
        return hash_32bit_ip(md5(str(pubk)), self.maproute.levels, self.maproute.gsize)