...
int andns_init(int restricted, char *resolv_conf,int family)
{
    ...
    for (i=0;i<_andns_ns_count_;i++) {
        ai= _andns_ns_[i];
        res= idp_inet_ntop(ai->ai_family, ai->ai_addr, buf, INET6_ADDRSTRLEN);
        if (!res) {
            strncat(msg,buf,INET_ADDRSTRLEN);
            strncat(msg,i==_andns_ns_count_-1?". ":", ",2);
        } 
        else
            error("In andns_init: error converting sockaddr -> %s.",\
                    strerror(errno));
        }
...
int dns_forward(dns_pkt *dp,char *msg,int msglen,char* answer)
{
    ...
    if ((res= d_p(dp_forward, fwdbuf))==-1) 
    { /* dp_foward is destroyed */
        error(err_str);
        goto safe_failing;
    }
...
char *andns_rslv(char *msg, int msglen,char *answer, int *answ_len)
{
    ...
    if (proto==NK_DNS) res= d_u(msg,msglen,&dp);
    else if (proto==NK_INET || proto==NK_NTK) res= a_u(msg,msglen,&ap);
    else {
        debug(DBG_INSANE,"andns_rslv(): Which language are you speaking?");
        return NULL;
    }

    if (res==0) goto discard;
        
    memset(answer, 0, ANDNS_MAX_SZ);
    if (res==-1) goto intrprt;
