...
char *tracer_pack_pkt(struct bcast_hdr *bcast_hdr, struct tracer_hdr *tracer_hdr, struct tracer_chunk *tracer)
{
	...
	memcpy(msg+sizeof(struct brdcast_hdr)+sizeof(struct tracer_hdr), tracer, 
			sizeof(struct tracer_chunk)*tracer_hdr->hops);
	return msg;
}
 ...
 * It then returns the number of bblock found. Remember to free bbl_hdr and bbl 
...
struct tracer_chunk *tracer_add_entry(map_node *node, struct tracer_chunk *tracer, int *hops)
{
 ...
 */
int tracer_store_pkt(struct tracer_chunk *tracer, u_short hops, void *bnode_block_start, size_t bblock_sz)
{
	struct bnode_hdr 	*bblock_hdr;
	struct bnode_chunk 	*bblock;	
	struct bnode_hdr 	**bblist_hdr=0;
		...
		if(!bb)
			...
			for(i=0; i<bb; i++) {
				if(bblist_hdr[i]->bnode >= MAXGROUPNODE || bblist_hdr[i]->bnode < 0)
					continue;
				node=node_from_pos(bblist_hdr[i]->bnode);
				node->flags|=MAP_BNODE;
				if((bm=map_find_bnode(me.cur_bmap, me.bmap_nodes, node))==-1) {
					me.bmap_nodes++;
					me.cur_bmap=xrealloc(me.cur_bmap, sizeof(map_bnode *) * me.bmap_nodes);
					me.cur_bmap[me.bmap_nodes-1].bnode=node;
					me.cur_bmap[me.bmap_nodes-1].links=bblist_hdr[i]->links;
					for(e=0; e<bblist_hdr[i]->links; e++) {
						...
						rnode_add(me.cur_bmap[me.bmap_nodes-1].gnode, &rn);
					}
				} else {
					/*TODO: CONTINUE continue here*(
