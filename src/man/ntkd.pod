...
B<-i> option. For example: B<ntkd> -i wlan0 -i eth0 -i wifi1 -i eth1
...
When the B<SIGHUP> signal is sent to B<ntkd>, the F<andna_hostnames> file
...
ntk-wifi(8), andna(8), iproute2(8), route(8)
