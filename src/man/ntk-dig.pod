...
=item 	B<"domain", "53", "53/udp", "domain/udp">

=back
are valid service/proto strings.

For example, this command asks for "depausceve" with service 80 and proto
tcp:

=over

=item 	B<ntk-dig -s http/tcp depausceve>

=back
and It will retrieve the IP that publishes HTTP pages for hostname
"depausceve".
...
the IP that registered the hostname. So, if you do not want SNSD delegations
