...
=item	B<-s>	B<--service=n>

Specify the SNSD service to search. See /etc/services, Default is 0.

=back

=over

=item	B<-p>	B<--protocol=tcp --protocol=udp>

Specify protocol for SNSD service. Default is tcp.
...
for the hostname (considering the protocol too). See /etc/services.
