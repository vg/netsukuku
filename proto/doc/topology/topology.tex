...
The memory limits are respected, because the CTP, once it jumps
over the border of a gnode, deletes the specific details that were useful by the nodes of
the gnode.\\
\newline
We'll now describe the rules of the Flat levels.\\
...
Detailed rules:
\begin{enumerate}
	\item When exploring the level $n$, each inferior level must be
		already explored.\\
	...
	\item The LTP respects the same rule of the QSPN v2: it is forwarded
		by a node only if it carries interesting information,
	...
	\item \label{rule6}
		...
		considerably,
		both $b$ and $b'$ send a CTP of level $n$ to their external
