...
By looking at many different simplification, we can recognize some general
rules:
\begin{enumerate}
	\item For each TP there has to be its inverse. For example, if there's
		a TP which covered the $12345$ route, then there has to be at
...
explore all the infinite combination of routes. For example:
\begin{figure}[h]
	\begin{center}
...
\begin{thebibliography}{99}
	...
	\bibitem{simrouteawk} Simplify Routes in Awk: \href{http://cvs.hinezumi.org/viewcvs/netsukuku/proto/qspn/simplify\_routes.awk}{simplify\_routes.awk}.
\end{thebibliography}
