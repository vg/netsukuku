...
exploration.\\
\newline
The natural solution is to totally re-explore the network each
time a change occurs. However, when the change affects only a local portion of the
network, a new total exploration becomes inefficient, because it will involve
even unaffected nodes.
It is then necessary to define an extension of the QSPN, which
will explore only the local part of the network affected by the change.
...
\subsection{Local QSPN}
\label{sec:localqspn}
The Local QSPN can be intuitively described as follo: 
\begin{quote}
	Suppose that a change occurred in the network. A Local CTP will
	explore only the portion of the network, which is formed by nodes
	interested in the change. This local zone becomes, for the CTP,
	as a separated network: the nodes adopts a separated QSPN cache
	for the CTPs restricted to this zone, and when a CTP reaches its
	boundaries, it will be reflected back, as if it reached the end of the
	network.
\end{quote}
TODO:
...
\begin{itemize}
	\item \verb|March 2007|
		\begin{itemize}
			...
			\item Description of the Local QSPN (sec.  \ref{sec:localqspn})
