...
%Let $r$ be a neighbor of $n$. When a TP traverse the link $r\rightarrow n$, the
...
\begin{description}
	\item[Worsened link] Suppose that the link $A
		...
		\item \label{stepR} 
			...
			\[
			R=\{r\in M_p\;|\;\exists q\in Q:\;\T{dst}(r)=\T{dst}(q)\}
			\]
			where $M_p$ are all the primary routes of the map, and 
		...
		update its map and forward the ETP to the other neighbours, as
		follow:
		\begin{enumerate}
