...
where a node won't change it's physical location quickly or often.\\

This assumption is licit because the location of a WiFi node mounted on
top of a building won't change and it's only dynamic actions would be the
joining and the disconnecting to and from the network and the changes of the
quality of it's links.\\
...
\begin{enumerate}
	...
	\item   The network isn't updated quickly: several minutes may be
		...
		instant using the routes of it's neighbours.
\end{enumerate}
...
Netsukuku implements it's own algorithm, the \emph{QSPN} (\textbf{Q}uantum
\textbf{S}hortest \textbf{P}ath \textbf{N}etsukuku). The name derives from the
way of working of it's principal component: the \emph{TP} (Tracer Packet), a
...
based: it's a packet which stores the ID's of the traversed hops in it's body.
...
receives a TP will forward it to all it's neighbours, except the one from which
...
will not forward the ATP to any of it's neighbours if it's node ID is already present
...
A node considers a received TP interesting when it's body contains at least a
new route. In other words, if a TP contains routes already known by the node,
it's considered uninteresting.\\

When a node receives an interesting TP, it forwards the packet to all it's
...
rule: a node drops the received ATP if it's node ID is already present in the route
...
\begin{description}
	...
	\item \label{backpropstep}
		If $S\neq \0$
		\begin{enumerate}
			\item $C$ creates a new ETP, appending the set $S$ and it's ID on it.
		...
		ETP, then $C$ sends back it's better routes which have the same destination of
	...
	\item 	Let \[\ove R=\pgra{r\in R\;|\;\nexists s\in S:\;dst(r)=dst(s)}\] 
		...
		$C$ packs the ETP with the set $\ove R$ and adds it's ID.
		The ETP is sent to all it's neighbours except $N$.\\
	...
	By step \ref{backpropstep}, $C$ sends back to $N$ it's known best route
	...
	\item[Broken link] \qq\\
		...
		\item $B$ fills the ETP: 
			\begin{enumerate}
				...
				\item $B$ appends it's ID.
				\item $B$ sets to 1 the \emph{flag of interest}.
			\end{enumerate}
		\item Finally, $B$ sends the ETP to all it's neighbours, except $A$.
		\end{enumerate}
...
%		\begin{enumerate}
%			\item Each neighbour $B_i$ sends it's whole map to
%				$A$
...
%				containing all the primary routes of it's
%				map.
