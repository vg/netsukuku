...
\begin{description}
	...
	\item For all $r' \in R'$,
		...
		\begin{enumerate}
			\item \label{ETPstep3a}
				...
				$\T{dst}(r')=\T{dst}(m')$. If $m'$
				exists and if $r'$ is a better alternative to $m'$,
				...
				case we'll have $r\leftarrow B\leftarrow
				A$.}),
	...
	\item[A node dies]
		...
		\begin{enumerate}
			...
			\item Note: the rule of step \ref{ETPstep3a} of the worsened
				link case implies that if $m'$ doesn't exist
				then $r'$ is removed from $R$. This strategy is adopted
				to discard misleading information regarding
				new routes, in fact, a node can know the
				existence of a new route only with the
				\textbf{new link} ETP packets. As an
				example, consider the following infinite loop
				that can happen without the use of this rule:
				\begin{enumerate}
					\item Suppose
						that two nodes $X,Y$ die almost at the
						same instant. Then two death-ETPs are
						generated, one for the death of $X$ and
