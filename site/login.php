...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	if (!$authorized)
		message($lang_login['Wrong user/pass'].' <a href="index.php?pag=forum&amp;act=login&amp;action=forget">'.$lang_login['Forgotten pass'].'</a>');
...
else if ($action == 'out')
{
	...
	redirect('index.php?pag=forum', $lang_login['Logout redirect']);
}
...
$redirect_url = (isset($_SERVER['HTTP_REFERER']) && preg_match('#^'.preg_quote($pun_config['o_base_url']).'/(.*?)\.php#i', $_SERVER['HTTP_REFERER'])) ? htmlspecialchars($_SERVER['HTTP_REFERER']) : 'index.php?pag=forum';
				...
				<fieldset>
					<legend><?php echo $lang_login['Login legend'] ?></legend>
						<div class="infldset">
							...
							<p><a href="index.php?pag=forum&amp;act=register" tabindex="4"><?php echo $lang_login['Not registered'] ?></a>&nbsp;&nbsp;
							<a href="index.php?pag=forum&amp;act=login&amp;action=forget" tabindex="5"><?php echo $lang_login['Forgotten pass'] ?></a></p>
						</div>
				</fieldset>
			</div>
