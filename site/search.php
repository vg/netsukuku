...
if (isset($_GET['action']) || isset($_GET['search_id']))
{
	...
	else
	{
		...
		if ($action != 'show_new' && $action != 'show_24h')
		{
			...
			header('Location: index.php?pag=forum&act=search&search_id='.$search_id);
			exit;
		}
	}
	...
	if ($search_results != '')
	{
		...
		$paging_links = $lang_common['Pages'].': '.paginate($num_pages, $p, 'index.php?pag=forum&amp;act=search&amp;search_id='.$search_id);
		...
		for ($i = 0; $i < count($search_set); ++$i)
		{
			...
			while (list(, $temp) = @each($forum_list))
			{
				if ($temp[0] == $search_set[$i]['forum_id'])
					$forum = '<a href="index.php?pag=forum&amp;act=viewforum&amp;id='.$temp[0].'">'.pun_htmlspecialchars($temp[1]).'</a>';
			}
			...
			if ($show_as == 'posts')
			{
				...
				$subject = '<a href="index.php?pag=forum&amp;act=viewtopic&amp;id='.$search_set[$i]['tid'].'">'.pun_htmlspecialchars($search_set[$i]['subject']).'</a>';
				...
				if ($search_set[$i]['poster_id'] > 1)
					$pposter = '<strong><a href="index.php?pag=forum&amp;act=profile&amp;id='.$search_set[$i]['poster_id'].'">'.$pposter.'</a></strong>';
...
<div class="blockpost searchposts<?php echo $vtbg ?>">
	<h2><?php echo $forum ?>&nbsp;&raquo;&nbsp;<?php echo $subject ?>&nbsp;&raquo;&nbsp;<a href="index.php?pag=forum&amp;act=viewtopic&amp;pid=<?php echo $search_set[$i]['pid'].'#p'.$search_set[$i]['pid'] ?>"><?php echo format_time($search_set[$i]['pposted']) ?></a></h2>
	<div class="box">
		<div class="inbox">
			<div class="postleft">
				<dl>
					...
					<dd><?php echo $icon; ?></dd>
					<dd><p class="clearb"><a href="index.php?pag=forum&amp;act=viewtopic&amp;pid=<?php echo $search_set[$i]['pid'].'#p'.$search_set[$i]['pid'] ?>"><?php echo $lang_search['Go to post'] ?></a></p></dd>
				</dl>
			</div>
			<div class="postright">
				<div class="postmsg">
...
<?php
			...
			else
			{
				...
				$subject = '<a href="index.php?pag=forum&amp;act=viewtopic&amp;id='.$search_set[$i]['tid'].'">'.pun_htmlspecialchars($search_set[$i]['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($search_set[$i]['poster']).'</span>';
				...
				if (!$pun_user['is_guest'] && $search_set[$i]['last_post'] > $pun_user['last_visit'])
				{
					...
					$subject_new_posts = '<span class="newtext">[&nbsp;<a href="index.php?pag=forum&amp;act=viewtopic&amp;id='.$search_set[$i]['tid'].'&amp;action=new" title="'.$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a>&nbsp;]</span>';
				}
				else
				...
				if ($num_pages_topic > 1)
					$subject_multipage = '[ '.paginate($num_pages_topic, -1, 'index.php?pag=forum&amp;act=viewtopic&amp;id='.$search_set[$i]['tid']).' ]';
				else
...
?>
				<tr<?php if ($item_status != '') echo ' class="'.trim($item_status).'"'; ?>>
					...
					<td class="tcr"><?php echo '<a href="index.php?pag=forum&amp;act=viewtopic&amp;pid='.$search_set[$i]['last_post_id'].'#p'.$search_set[$i]['last_post_id'].'">'.format_time($search_set[$i]['last_post']).'</a> '.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($search_set[$i]['last_poster']) ?></td>
				</tr>
<?php
...
<div id="searchform" class="blockform">
	...
	<div class="box">
		<form id="search" method="get" action="index.php">
			<input type="hidden" name="pag" value="forum">
